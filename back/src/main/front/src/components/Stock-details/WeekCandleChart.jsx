...
const WeekCandleChart = ({ stockCode }) => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                ...
                const startDate = new Date(today.setFullYear(today.getFullYear() - 2)) // 최근 2년치 주봉
                    .toISOString()
                    .slice(0, 10)
                    .replace(/-/g, '');

                const response = await axios.get('/time/stocks/week', {
                    params: {
                        stockCode,
                        startDate,
                        endDate,
    ...
    const series = [{
        data: sortedData.map(item => ({
            x: `${item.date.slice(0, 4)}-${item.date.slice(4, 6)}-${item.date.slice(6, 8)}`,
            y: [
...
export default WeekCandleChart;
