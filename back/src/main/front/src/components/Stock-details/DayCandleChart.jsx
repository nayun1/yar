...
const DayCandleChart = ({ stockCode }) => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                ...
                const startDate = new Date(today.setFullYear(today.getFullYear() - 1)) // 최근 1년치
                    .toISOString()
                    .slice(0, 10)
                    .replace(/-/g, '');

                const response = await axios.get('/time/stocks/day', {
                    params: {
                        stockCode,
                        startDate,
                        endDate,
